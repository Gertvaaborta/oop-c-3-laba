...
void Array::copyFigures(const Array& other) {
    ...
    for (const auto& figure : other.figures) {
        if (dynamic_cast<const Rhombus*>(figure.get())) {
            figures.push_back(std::make_unique<Rhombus>(*dynamic_cast<const Rhombus*>(figure.get())));
        } else if (dynamic_cast<const Pentagon*>(figure.get())) {
